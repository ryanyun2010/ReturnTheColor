...
class Enemy {
    constructor(damage,health,x,y,w,h){
        this.health = health;
        this.damage = damage;
        this.x = x;
        this.y = y;
    ...
    check(){
        ...
        if(getDistance(this.x,this.y,player.x,player.y) < this.w / 2 + 25){
            health -= this.damage;
            if(health < 1){
                health = 0;
...
function spawnWaveEnemies(){
    for(var i = 0; i < wave * 10; i++){
        enemies.push(new Enemy(0.01,1,Math.random()*700,Math.random() * 700,50,50)) 
    }
}

function preload(){
    cardpng = loadImage("img/card.png")
}
function setup() {
...
function draw() {
    ...
    fill("white")
    rect(20,20,colorrestored * 320,50)
    fill("black");
    noStroke();
    rect(360,20,320,50)
    fill(80 + 175 * colorrestored,80 - 80 * colorrestored,80 - 80 * colorrestored)
    rect(360,20,(health/maxhealth) * 320,50)
    fill("white");
    textSize(40);
    textAlign(CENTER)
    textStyle(BOLD)
    text("Wave "+ wave,0,80,700);
}
